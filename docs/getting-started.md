...
명세 주도 개발(Spec-Driven Development)을 위한 방법론으로, IDE에 종속되지 않고 AI 어시스턴트와 효율적으로 협업할 수 있도록 설계되었습니다.

**핵심**: 코드 작성 전 명세(SPEC.md) → 계획(PLAN.md) → 작업 분해(tasks.md) → 구현 순서로 진행합니다.
